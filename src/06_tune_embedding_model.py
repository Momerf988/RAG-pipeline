# Embedding model ablation -- answers one question: is the current embedder (MiniLM)
# capping retrieval quality, or would a stronger embedder not actually help?
#
# No LLM calls, no cost: pure local encoding + cosine similarity + the same cross-encoder
# reranker used in production. No Pinecone writes either, fully offline via numpy against
# the gold character spans built by 05_spanify_gold_answers.py. Chunking is copied verbatim
# from document_processor.py -- only the embedding model changes between the two runs.
#
# Decision rule, declared before looking at the numbers: switch to bge-base-en-v1.5 only if
# overall recall improves by >= +0.05, or R4 recall improves by >= +0.10. Otherwise keep
# MiniLM -- that's a legitimate, reportable result, not a failed experiment.
...
GOLD_SPANS_FILE = "benchmark_datasets/gold_spans.json"

CANDIDATES = [
    config.TRANSFORMER_MODEL,     # incumbent -- whatever is actually configured
    "BAAI/bge-base-en-v1.5",      # the honest upgrade candidate to test against it
]
...
KEEP_TOP = 7            # this ablation predates the keep_top=15 decision, kept at the
                         # value that was live when it was run


# ---- chunking, copied verbatim from document_processor.py so this stays a fair test of
# the embedder alone, with nothing else in the pipeline changing between the two runs ----
...
def run_model(model_name, chunk_ids, chunk_texts, offsets_by_chunk_id, bench_rows, gold_spans, reranker):
    ...
    for spec_id, gold in gold_spans.items():
        ...
        top_k_ids = [cid for _, cid in reranked]

        # Only chunks from the same source file are meaningful for overlap -- offsets are
        # file-local, so comparing across files would compare unrelated coordinate systems.
...
if __name__ == "__main__":
    ...
    for model_name in CANDIDATES:
        results = run_model(model_name, chunk_ids, chunk_texts, offsets_by_chunk_id, bench_rows, gold_spans, reranker)
    ...
    print("\n" + "=" * 60)
    print("DECISION (pre-declared rule):")
