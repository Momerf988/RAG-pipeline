# Keep_top quality ablation, System A only, k in {7, 15, 20}.
#
# 07_tune_retrieval_depth.py already showed recall keeps climbing well past the point where
# the pre-declared +0.05 materiality bar is cleared. Recall alone can't say whether that
# extra retrieved text actually helps the tutor's real answer or just adds noise the
# generator has to sift through, so this closes the gap with real generation + RAGAS scoring.
#
# Why System A only, not System B: keep_top is retrieval-stage config, decided before either
# system's generation logic runs -- it's shared infrastructure, not a per-system setting.
# Testing it via System A means the result doesn't depend on which model System B's
# evaluator uses, and whatever keep_top this settles on is what both systems use in the
# final 150-row run.
#
# Why k = 7, 15, 20: k=7 is the original production setting, kept in for a direct
# before/after comparison. k=15 and k=20 are the practical contenders from
# 07_tune_retrieval_depth.py's curve -- k=10 only barely clears the materiality bar, while
# 15/20 clear it by a much larger margin, and this ablation is what decides whether that
# margin is a real benefit or just more text. Larger k values aren't tested here: at
# ~175 tokens/chunk, k=40 alone is already ~7,000 tokens of context, which risks exceeding
# the account's TPM cap in a single generation call -- fine for the zero-cost recall
# diagnostic, not a safe setting to actually generate answers with.
#
# Sample: 24 questions, 6 per retrieval level (R1-R4), fixed seed for reproducibility -- not
# the full 150, to keep this a small, cheap check rather than a repeat of the full RAGAS
# run. The same 24 questions are reused across all three k values, so the comparison is
# paired (same questions, different k) rather than three independent samples averaged.
#
# This calls RAG_response_processor's existing methods directly with keep_top overridden
# per run -- respond_to_user() itself, and its default keep_top, are untouched.
...
RANDOM_SEED = 42
SLEEP_BETWEEN_ROWS = 8          # llama-3.1-8b-instant is TPM-bottlenecked
...
def generate_at_k(sample, k, user_query_processor, llm_response_processor):
    output_file = f"results/keep_top_ablation_k{k}.csv"
...
def score_with_ragas(input_file, k):
    output_file = f"results/keep_top_ablation_k{k}_scorecard.csv"
...
if __name__ == "__main__":
    ...
    print(summary_df)
    summary_df.to_csv("results/keep_top_ablation_summary.csv")
    print("\nSaved to results/keep_top_ablation_summary.csv")
    print("\nNOTE: this settles keep_top for both System A and System B (shared retrieval")
    print("config). It does not test how keep_top affects System B's evaluator verdicts --")
    print("that interaction is a disclosed scope simplification, not silently assumed away.")
