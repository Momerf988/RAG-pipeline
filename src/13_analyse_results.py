...
# Builds a browsable Excel workbook comparing System A and System B across every angle
# used in the write-up: overall, non-fallback-only, by topic, by Bloom level, by
# difficulty, by retrieval level, by question type, CRAG path breakdown, and a per-item
# audit of the rows System B abstained on. For the significance-tested version of these
# same comparisons, see 14_verify_statistics.py -- this script is for exploring the data,
# that one is for checking the report's claims.

BENCHMARK_FILE = "benchmark_datasets/benchmark_specification_matrix.xlsx"
SHEET_NAME = "Benchmark Spec Matrix"
SYSTEM_A_SCORECARD = "results/System_A_final_scorecard.csv"
SYSTEM_B_SCORECARD = "results/System_B_3way_scorecard.csv"
SYSTEM_B_EVAL_RESULTS = "results/System_B_3way_eval_results.csv"
...
def fair_comparison_non_fallback(a, b):
    """Compares A and B only on rows where B did not fall back -- isolates whether CRAG's
    answer quality changed on rows it actually answered, without the fallback rows (which
    correctly score near 0) dragging B's average down."""
...
def fallback_row_analysis(a, b):
    """The key question for each row System B abstained on: was System A's answer actually
    faithful (meaning CRAG likely over-rejected) or was it hallucinating (meaning the
