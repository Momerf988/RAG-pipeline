# Retrieval-depth (keep_top) sweep -- answers "would keeping more chunks after reranking
# actually retrieve more of the evidence a question needs?" with a measured recall curve
# instead of a guess.
#
# For each of the 150 benchmark questions, retrieves the same top-60 candidates, reranks
# all of them once (not once per k), then computes span-recall at several prefix lengths of
# that single reranked list: k = 3, 5, 7, 10, 15, 20, 60. One pass gives the full
# recall-vs-k curve instead of six separate runs. Uses the embedder already settled by
# 06_tune_embedding_model.py. No LLM calls, no cost, no Pinecone writes.
#
# Decision rule, declared before looking at the numbers: raising keep_top above 7 is only
# worth adopting if some k > 7 improves overall recall by >= +0.05. Otherwise keep keep_top
# = 7 -- also a valid, reportable outcome. This script only measures recall; whether the
# recall gain actually translates into better generated answers is checked separately in
# 08_tune_keep_top_quality.py.
...
CURRENT_KEEP_TOP = 7    # the value live in RAG_response_processor.rerank at the time this ran


# ---- chunking, copied verbatim from document_processor.py ----
...
if __name__ == "__main__":
    ...
    print(f"\nEncoding {len(chunk_texts)} chunks with {config.TRANSFORMER_MODEL} "
          f"(already-settled embedder from 06_tune_embedding_model.py)...")
    ...
    results_by_k = {k: [] for k in K_VALUES}  # k -> list of (spec_id, r_level, recall)
