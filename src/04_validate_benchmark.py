...
# Validates benchmark_datasets/benchmark_specification_matrix.xlsx in two passes:
#   1. Whole-dataset sanity check (cheap, all rows): required fields present, reference
#      length, leakage phrases, near-duplicate questions.
#   2. Per-topic deep validation (run in batches, e.g. T1-T3 first): extracts every fenced
#      code block from the question + answer text and actually executes it. For Debugging
#      questions, confirms the code really raises an exception, and checks the exception
#      type named in the text against what actually happens.
...
def whole_dataset_sanity(rows, idx):
    ...
    # Near-duplicate check across all questions, TF-IDF cosine similarity, local and free.
    if len(texts) > 1:
...
def is_fragment(block):
    ...
    placeholder (Code Completion questions use '___'), or too short/bare to be a real
    program (a lone expression or a signature with no body, shown for illustration only)."""
    if "___" in block:
        return True
...
def deep_validate_topics(rows, idx, topics):
    ...
    for r in subset:
        ...
        if not real_blocks:
            results.append((spec_id, qtype, "SKIP", "only fill-in-blank / fragment code found -- nothing executable to check"))
            continue
        ...
        # answers show a sequence of snippets that build on each other (a later block reuses
        # a variable the earlier block defined). Testing them isolated produces spurious
        # NameErrors that aren't a real content bug.
        concat_code = "\n".join(real_blocks)
        try:
        ...
        if qtype == "Debugging":
            ...
            # the concatenated run nor any individual block produces it. A debugging question
            # about a silent logic bug (wrong output, no crash) is legitimate and can't be
            # checked this way, so those aren't flagged.
            if concat_ok and not named:
                row_notes.append("INFO: code ran cleanly, no exception claimed in text -- likely a silent logic bug (can't auto-verify), not flagged")
                status = "PASS"
            elif not named:
                ...
                # Named an exception but the concatenated run doesn't reproduce it -- try
                # each block individually before concluding it's a real mismatch.
                per_block_hits = []
                ...
                if any(e in named for e in per_block_hits):
                    row_notes.append(f"OK: one block individually raised {named}, matches text (concatenated run masked it)")
                    status = "PASS"
                else:
                    row_notes.append(f"FAIL: text names {named} but neither concatenated nor individual blocks raised it (got {per_block_hits or concat_exc})")
                    status = "FAIL"
        else:
            if concat_ok:
                ...
                row_notes.append("FAIL: code does not parse even when blocks are concatenated in order -- likely a real syntax problem")
                status = "FAIL"
            else:
                row_notes.append(f"FAIL: non-Debugging code raised {concat_exc} even when blocks concatenated in order -- {concat_stderr}")
                status = "FAIL"
