...
GENERATOR_MODEL = "openai/gpt-oss-120b" # new model for improved performance

# How many chunks to pull from the DB per Retrieval_Level row
...
# PHASE 4: Generate the RAGAS ground-truth benchmark dataset, one topic at a time
class BenchmarkQuestionGenerator:
 ...
 #       # Vector IDs were written as f"{pdf_file}_{i}" during ingestion, so the
 #       # PDF name is a usable prefix -- no need to touch the ingestion schema.
 ...
 #       return chunk_pool
    ...
    def get_topic_chunk_pool(self, source_pdf):
            ...
            for id_batch in self.index.list(prefix=prefix):
                ...
                # FIXED: Use attribute access (.vectors) instead of dictionary access (['vectors'])
                for vector_id, vector_data in fetched.vectors.items():
                    
                    # FIXED: Use attribute access for metadata as well
                    chunk_pool[vector_id] = vector_data.metadata['line']
                    
            return chunk_pool

#    def pick_chunks(self, chunk_pool, retrieval_level):
#        chunk_ids = list(chunk_pool.keys())
#        chunk_count = RETRIEVAL_CHUNK_COUNTS.get(retrieval_level, 1)
#        chunk_count = min(chunk_count, len(chunk_ids))
#        sampled_ids = random.sample(chunk_ids, chunk_count)
#        sampled_text = [chunk_pool[cid] for cid in sampled_ids]
#        return sampled_text, sampled_ids

    def pick_chunks(self, chunk_pool, retrieval_level):
        # sort by the numeric suffix in the vector ID (e.g. "..._14" -> 14) so
        # "sequential" actually means sequential in the source document
...
'''
    ...
    def generate_response(self, system_persona, user_instruction):
        response = self.llm_client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=65500,                             # allow long R4 Evaluate outputs
            response_format={"type": "json_object"},    # force valid JSON, catches parse-flakes
        )
...
# To fix code not adding full in JSON:
#    def parse_generated_json(self, raw_text):
#            cleaned = raw_text.strip()
#            if cleaned.startswith("```"):
#                cleaned = cleaned.strip("`")
#                cleaned = cleaned.replace("json", "", 1).strip()
#                
#            # FIXED: strict=False allows the JSON parser to accept raw newlines and tabs 
#            # that the LLM often puts inside code snippets
...
#            return parsed["question"], parsed["answer"], parsed["section"]
    ...
    def generate_for_topic(self, topic_id):
        ...
        # Column positions (1-indexed), matching the locked header row
        ...
        for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row):
            ...
            try:
                ...
                skipped_count += 1
                print(f"{spec_id}: skipped -- {type(e).__name__}: {e}")   # show exception class
