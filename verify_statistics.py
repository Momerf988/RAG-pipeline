"""
verify_statistics.py  —  reproduces every statistic reported in COM748_Full_Report.docx
Run:  python verify_statistics.py
...
for label, df in [("All items", d), ("Substantive answers", d[d.path_taken != 'fallback'])]:
    print(f"--- Table 4: {label} (n={len(df)}) ---")
    for m in M:
        a, b = df[m + '_A'], df[m + '_B']
...
# ---- Table 5 -------------------------------------------------------------
print("--- Table 5: complexity gradient ---")
...
# ---- Section 4.3: R4 alone + Spearman ------------------------------------
print("\n--- Section 4.3: R4 alone (n=12) ---")
...
print("\n--- Section 4.3: Spearman rho, gradient vs effect size ---")
